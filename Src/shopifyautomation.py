...
def get_orders_with_specific_fields(page_limit):
	shopify_api_key = access.shopify_api_key()
	shopify_password = access.shopify_password()
	shopify_url = access.shopify_url()
	base_url = f"https://{shopify_api_key}:{shopify_password}@{shopify_url}/admin/api/2024-01/"
	...
	desired_fields = [
		'name', 'created_at', 'customer.id', 'customer.first_name', 'customer.last_name',
		'source_name', 'total_price', 'financial_status', 'fulfillment_status',
		'line_items', 'shipping_lines.title', 'tags'
	]

	# Join fields for the fields parameter
	fields_param = ','.join(desired_fields)
	rows_limit=250
	endpoint = f"orders.json?limit={rows_limit}&status=any&fields={fields_param}"
...
def check_shopify_api_call_limit():
	shopify_api_key = access.shopify_api_key()
	shopify_password = access.shopify_password()
	shopify_url = access.shopify_url()
	base_url = f"https://{shopify_api_key}:{shopify_password}@{shopify_url}/admin/api/2024-01/"
	endpoint = "shop.json"  # A lightweight endpoint
	...
	if response.status_code == 200:
		rate_limit = response.headers.get('X-Shopify-Shop-Api-Call-Limit')
		print(f"Current API call limit: {rate_limit}")
		return rate_limit
	else:
		print(f"Failed to retrieve shop information, status code: {response.status_code}")
		return None
